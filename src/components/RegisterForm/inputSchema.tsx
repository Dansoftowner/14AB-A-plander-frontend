...
export const regSchema = (t: TFunction<'register', undefined>) => z.object({
    username: z.string().min(5, { message: t('zodUsername') }).max(20)
                .regex(/^[a-zA-Z0-9\.]+$/g, { message: t('zodInvalidName') }),
    password: z.string().min(8, { message: t('zodPasswordLength') }).refine((str) => /[A-Z]/.test(str), { message: t('zodPassword') })
                .refine(str => /[0-9]/.test(str), { message: t('zodPassword') }),
